...
class MTCategoriesViewController: UIViewController {
    ...
    private func getCategoriesForCollectionView() {
        ...
        MTNetworkManager.shared.getPayload(at: "https://www.themealdb.com/api/json/v1/1/categories.php") { [weak self] (result: Result<MTCategoriesResponse, MTNetworkingError>) in
            DispatchQueue.main.async {
                switch result {
                case .success(let categoriesResponse):
                    let categories = categoriesResponse.categories.sorted { $0.name.lowercased() < $1.name.lowercased() }
