...
class MTMealsViewController: UIViewController {
    ...
    private func getMeals() {
        ...
        MTNetworkManager.shared.getPayload(at: "https://www.themealdb.com/api/json/v1/1/filter.php?c=\(category.name)") { [weak self] (result: Result<MTMealsResponse, MTNetworkingError>) in
            DispatchQueue.main.async {
                self?.removeLoadingView()
                switch result {
                case .success(let mealsResponse):
                    self?.meals = mealsResponse.meals.sorted(by: { $0.name.lowercased() < $1.name.lowercased() })
